...
export default function CreateTestScreen() {
  ...
  const handleSaveAsDraft = async () => {
    ...
    try {
      ...
      await adminAPI.createTest(requestData);
      showSuccess("Test uğurla yaradıldı");
      router.replace("/admin/tests");
    } catch (error) {
      handleApiError(error, "Test yaradılarkən xəta baş verdi");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const response = await adminAPI.createTest(requestData);
      await adminAPI.publishTest(response.id);
      showSuccess("Test uğurla yaradıldı və nəşr edildi");
      router.replace("/admin/tests");
    } catch (error) {
